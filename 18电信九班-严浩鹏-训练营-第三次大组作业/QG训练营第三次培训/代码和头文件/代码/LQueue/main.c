...
int main()
{
	...
	while (1)
	{
		...
		case 1:
			...
			{
				pt = malloc(sizeof(LQueue));  //给队列指针pt赋地址
				InitLQueue(pt);
				printf("初始化成功\n");
			}
			break;
		...
		case 3:
			if (pt == NULL)
				printf("传入地址为空\n");
			else if (IsEmptyLQueue(pt))
				printf("队列为空\n");
			else
				printf("队列不为空\n");
			break;
		case 4:
			if (GetHeadLQueue(pt, &frontdata))  //传入frontdata的指针以修改frontdata的指向
			{
				printf("队头的元素为:");
