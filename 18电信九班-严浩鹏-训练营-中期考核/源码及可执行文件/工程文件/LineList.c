...
Status GetBookInf(BookInf *Book)
{
	...
	Book->BookID = Bookpt->Book.SpareNum;   //为新书分配书号
...
Status AddBook(BookNode *p, BookInf AddedBook)
{
	...
	if (p->next->Book.TotalNum == 0)   //若为第一本书，将其存储在头的下一节点
	{
...
{
	while (p != NULL&&strcmp(p->Book.BookName,BookName))   //直到找到输入的书的名字
	{
		p = p->next;
	}
	if (p == NULL)   //没有找到输入的书
	...
	{                                  //输出相关信息
		printf("%s 书号是", p->Book.BookName);
		fprintf(Logfp, "%s%s", p->Book.BookName, " 书号是");
		printf("%d\n", p->Book.BookID);
		fprintf(Logfp, "%d%c", p->Book.BookID, '\n');
		printf("-----------------------------\n");
		printf("作者为 %s\n", p->Book.Author);
		printf("-----------------------------\n");
		fprintf(Logfp, "%s", "-----------------------------\n");
		fprintf(Logfp, "%s%s%c", "作者为 ", p->Book.Author, '\n');
		printf("剩余数目为 %d", p->Book.SpareNum);
		fprintf(Logfp, "%s%d", "剩余数目为  ", p->Book.SpareNum);
		printf("   总共数目为 %d\n", p->Book.TotalNum);
		fprintf(Logfp, "%s%d%c", "   总共数目为 ", p->Book.TotalNum, '\n');
		printf("-----------------------------\n");
		fprintf(Logfp, "%s", "-----------------------------\n");
		if (p->Reader != NULL)
		{
			printf("第一个借书且未归还的是 %lld\n", p->Reader->ReaderID);
			fprintf(Logfp, "%s%lld%c", "第一个借书且未归还的是 ", p->Reader->ReaderID, '\n');
			printf("应该在 %lld 前归还此书\n", p->Reader->ReturnDate);
			fprintf(Logfp, "%s%lld%s", "应该在 ", p->Reader->ReturnDate, " 前归还此书\n");
		}
		else
		{
			printf("还没有人在借这本书\n");
			fprintf(Logfp, "%s", "还没有人在借这本书\n");
		}
		printf("-----------------------------\n");
		fprintf(Logfp, "%s", "-----------------------------\n");
		if (p->AppointerQueue == NULL)
		{
			printf("还没有在预约此书\n");
			fprintf(Logfp, "%s", "还没有在预约此书\n");
		}
		else
		{
			printf("有%u位读者预约此书\n", p->AppointerQueue->length);
			fprintf(Logfp, "%s%u%s", "有", p->AppointerQueue->length, "位读者预约此书\n");
		}
		printf("-----------------------------\n");
		fprintf(Logfp, "%s", "-----------------------------\n");
	}
