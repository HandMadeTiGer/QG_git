...
int main()
{
	if ((Bookfp = fopen("../可执行文件/Book", "rb+")) == NULL)   
	{
		printf("无法打开文件\n");
		exit(0);
	}
	if ((Logfp = fopen("../可执行文件/Log", "a")) == NULL)
	{
		printf("无法打开文件\n");
		exit(0);
	}
	if ((Readerfp = fopen("../可执行文件/Reader", "rb+")) == NULL)
	{
		printf("无法打开文件\n");
		exit(0);
	}
	if ((Queuefp = fopen("../可执行文件/Queue", "rb+")) == NULL)
	{
		printf("无法打开文件\n");
		exit(0);
	}
				...
				switch (Cho)
				{
					...
					case 2:
						...
						if (DelBook(Bookpt, StrInp, Book))
						{
							...
							Bookpt->Book.SpareNum--;  //减少书库中数目种类
							rewind(Bookfp);
							fwrite(&(Bookpt->Book), sizeof(BookInf), 1, Bookfp);
							fseek(Bookfp, temp, 0);
